...
function Set-RegValue {
    ...
    Write-Log ("Set {0}\{1} ({2}) = {3}" -f $RelativePath, $Name, $Type, $Value)
}
...
$script:LogPath = Join-Path $logDir ("apply-qcpil8280-full-extension-registry-{0}.log" -f $stamp)
$backupDir = Join-Path $backupRoot ("qcpil8280-full-extension-registry-pre-apply-{0}" -f $stamp)
New-Item -ItemType Directory -Force -Path $backupDir | Out-Null

Write-Log "Applying qcpil8280 full qcpilEXT8280 registry experiment."
Write-Log "WindowsDrive: $WindowsDrive"
Write-Log "BackupDir: $backupDir"

$root = Resolve-OfflineWindowsRoot -Drive $WindowsDrive
$systemHive = Join-Path $root "Windows\System32\Config\SYSTEM"
$hiveName = "ALIOTH_QCPIL_FULL_EXT_SYSTEM_$PID"
$loaded = $false
...
try {
    ...
    $values = @(
        [pscustomobject]@{ RelativePath = "$deviceRelative\SubsystemLoad\VENUS"; Name = "MemoryAlignment"; Type = "REG_DWORD"; Value = "0x00000000" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\SubsystemLoad\VENUS"; Name = "MemoryReservation"; Type = "REG_DWORD"; Value = "0x00500000" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\SubsystemLoad\GFXSUC"; Name = "MemoryAlignment"; Type = "REG_DWORD"; Value = "0x00001000" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\SubsystemLoad\GFXSUC"; Name = "MemoryReservation"; Type = "REG_DWORD"; Value = "0x00005000" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\PilConfig"; Name = "HypProtectionEnabled"; Type = "REG_DWORD"; Value = "0x00000001" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\PilConfig"; Name = "DoNotReturnMemoryToHLOS"; Type = "REG_DWORD"; Value = "0x00000001" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\PGCM"; Name = "BaseAddress"; Type = "REG_DWORD"; Value = "0x86700000" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\PGCM"; Name = "Size"; Type = "REG_DWORD"; Value = "0x07D00000" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\IMEM"; Name = "BaseAddress"; Type = "REG_DWORD"; Value = "0x146BF000" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\IMEM"; Name = "Offset"; Type = "REG_DWORD"; Value = "0x0000094C" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\DPOP"; Name = "GUID"; Type = "REG_SZ"; Value = "{ED9E8101-05FA-46B7-82AA-8D58770D200B}" }
        [pscustomobject]@{ RelativePath = "$deviceRelative\MSAL"; Name = "Type"; Type = "REG_DWORD"; Value = "0x00000001" }
    )

    $backup = foreach ($entry in $values) {
        Get-RegistryValueBackup -HiveRoot $hiveRoot -RelativePath $entry.RelativePath -Name $entry.Name -Type $entry.Type
    }
    ...
    $manifest = [pscustomobject]@{
        ...
        SourceInf = "qcpilEXT8280.inf"
        BackupDir = $backupDir
        CreatedAt = (Get-Date).ToString("s")
        RollbackCommand = "powershell -NoProfile -ExecutionPolicy Bypass -File `"$projectRoot\tools\Rollback-Qcpil8280FullExtensionRegistry-Admin.ps1`" -WindowsDrive $WindowsDrive -BackupDir `"$backupDir`""
    }
    $manifest | ConvertTo-Json -Depth 5 | Out-File -LiteralPath (Join-Path $backupDir "manifest.json") -Encoding UTF8

    Write-Log "qcpil8280 full qcpilEXT8280 registry experiment applied."
    Write-Log "Rollback command: $($manifest.RollbackCommand)"
    Write-Host "qcpil8280 full qcpilEXT8280 registry experiment applied."
