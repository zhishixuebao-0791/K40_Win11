...
BUILD_LOG="$LOG_DIR/mu-alioth-phase11-build-$(date +%Y%m%d-%H%M%S).log"
...
log "Applying phase11 ACPI patch: keep Phase10 HIDs and shrink ADSP/CDSP/SCSS _DEP to isolate ARPC/SSDD dependency gating"
python3 - "$MU_ROOT" <<'PY'
...
# Phase11 is intentionally narrow: match the dependency shape of SPSS, which is
...
phase11_providers = [r"\_SB_.PEP0", r"\_SB_.PILC", r"\_SB_.RPEN"]
for device in ["ADSP", "CDSP", "SCSS"]:
    text = replace_top_level_dep(text, device, phase11_providers)
...
if text != original:
    backup = asl.with_name(f"DSDT.asl.pre-phase11-{stamp}.bak")
...
checks = {
    ...
    "phase11_adsp_dep": r"Device(ADSP)" in text and r"\_SB_.ARPC" not in find_device_block(text, "ADSP")[2].split('Name(_HID', 1)[0],
    "phase11_cdsp_dep": r"Device(CDSP)" in text and r"\_SB_.ARPC" not in find_device_block(text, "CDSP")[2].split('Name(_HID', 1)[0],
    "phase11_scss_dep": r"Device(SCSS)" in text and r"\_SB_.ARPC" not in find_device_block(text, "SCSS")[2].split('Name(_HID', 1)[0],
...
for key, value in checks.items():
    print(f"{key}: {value}")
    if not value:
        raise SystemExit(f"phase11 ASL verification failed: {key}")
PY
...
for needle in expected_present:
    ...
    if not present:
        raise SystemExit(f"phase11 AML verification failed, missing {needle.decode()}")
for needle in expected_absent:
    present = needle in data
    print(f"AML contains stale {needle.decode()}: {present}")
    if present:
        raise SystemExit(f"phase11 AML verification failed, stale {needle.decode()}")
print(f"AML MTP08280 count: {data.count(b'MTP08280')}")
for forbidden in [b"\\_SB_.ARPC"]:
    print(f"AML contains phase11 forbidden provider token {forbidden!r}: {forbidden in data}")
PY
...
DEST="$OUTPUT_DIR/Mu-alioth-1-acpi-hid-phase11-$(date +%Y%m%d-%H%M%S).img"
