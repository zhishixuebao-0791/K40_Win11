...
$script:LogPath = Join-Path $logRoot ("audio-adsp-phase1b-apply-" + (Get-Date -Format "yyyyMMdd-HHmmss") + ".log")

Write-Log "Applying Alioth audio ADSP phase 1b."

if (-not (Test-Path -LiteralPath $packageRoot)) {
    $prepare = Join-Path $PSScriptRoot "prepare-audio-adsp-phase1b.ps1"
...
if (-not $SkipDism) {
    ...
    Write-Log "SkipDism was set; not adding driver package."
}

$hiveName = "ALIOTH_AUDIO_ADSP_SYSTEM_$PID"
...
try {
    ...
    foreach ($controlSet in $controlSets) {
        Add-CompatibleId -ControlSetPath $controlSet.PSPath -AcpiId "ACPI\QCOM051D" -AliasId "ACPI\QCOM251D" -Changes $changes
    }

    $backupPath = Join-Path $logRoot ("audio-adsp-phase1b-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss") + ".json")
    [ordered]@{
...
Write-Log "Phase-1b ADSP alias apply completed. Boot Mu-alioth and run Trace-AudioAliasState.ps1."
