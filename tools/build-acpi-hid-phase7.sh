...
BUILD_LOG="$LOG_DIR/mu-alioth-phase7-build-$(date +%Y%m%d-%H%M%S).log"
...
log "Applying phase7 ACPI HID patch to ASL and AML"
python3 - "$MU_ROOT" <<'PY'
...
hid_replacements = [
    ...
    ("QCOM057C", "QCOM257C"),
    ("QCOM058B", "QCOM258B"),
    # Phase7 pivots SSDD from the Phase6 2522 path to the official 8280 package HID.
    ("QCOM0522", "QCOM0620"),
    ("QCOM2522", "QCOM0620"),
]
...
if text != original_text:
    backup = asl.with_name(f"DSDT.asl.pre-phase7-{stamp}.bak")
...
if data != original_data:
    backup = aml.with_name(f"DSDT.aml.pre-phase7-{stamp}.bak")
...
for needle in expected_present:
    ...
    if not present:
        raise SystemExit(f"phase7 AML verification failed, missing {needle.decode()}")
for needle in expected_absent:
    present = needle in final
    print(f"AML contains {needle.decode()}: {present}")
    if present:
        raise SystemExit(f"phase7 AML verification failed, stale {needle.decode()}")
PY
...
DEST="$OUTPUT_DIR/Mu-alioth-1-acpi-hid-phase7-$(date +%Y%m%d-%H%M%S).img"
