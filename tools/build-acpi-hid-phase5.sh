...
BUILD_LOG="$LOG_DIR/mu-alioth-phase5-build-$(date +%Y%m%d-%H%M%S).log"
...
log "Applying phase5 ACPI HID patch to ASL and AML"
python3 - "$MU_ROOT" <<'PY'
from pathlib import Path
...
if text != original_text:
    backup = asl.with_name(f"DSDT.asl.pre-phase5-{stamp}.bak")
...
if data != original_data:
    backup = aml.with_name(f"DSDT.aml.pre-phase5-{stamp}.bak")
...
expected_present = [b"QCOM251B", b"QCOM2533", b"QCOM250B", b"QCOM258D", b"QCOM250E", b"QCOM257C", b"QCOM258B"]
expected_absent = [b"QCOM051B", b"QCOM0533", b"QCOM050B", b"QCOM058D", b"QCOM050E", b"QCOM057C", b"QCOM058B"]
final = aml.read_bytes()
for needle in expected_present:
    present = needle in final
    print(f"AML contains {needle.decode()}: {present}")
    if not present:
        raise SystemExit(f"phase5 AML verification failed, missing {needle.decode()}")
for needle in expected_absent:
    present = needle in final
    print(f"AML contains {needle.decode()}: {present}")
    if present:
        raise SystemExit(f"phase5 AML verification failed, stale {needle.decode()}")
PY
...
DEST="$OUTPUT_DIR/Mu-alioth-1-acpi-hid-phase5-$(date +%Y%m%d-%H%M%S).img"
