...
BUILD_LOG="$LOG_DIR/mu-alioth-phase6-build-$(date +%Y%m%d-%H%M%S).log"
...
log "Applying phase6 ACPI HID patch to ASL and AML"
python3 - "$MU_ROOT" <<'PY'
from pathlib import Path
...
if text != original_text:
    backup = asl.with_name(f"DSDT.asl.pre-phase6-{stamp}.bak")
...
if data != original_data:
    backup = aml.with_name(f"DSDT.aml.pre-phase6-{stamp}.bak")
...
for needle in expected_present:
    ...
    if not present:
        raise SystemExit(f"phase6 AML verification failed, missing {needle.decode()}")
for needle in expected_absent:
    present = needle in final
    print(f"AML contains {needle.decode()}: {present}")
    if present:
        raise SystemExit(f"phase6 AML verification failed, stale {needle.decode()}")
PY
...
DEST="$OUTPUT_DIR/Mu-alioth-1-acpi-hid-phase6-$(date +%Y%m%d-%H%M%S).img"
